...
def main():
    ...
    filename = "test_inputs/katana.txt"
    encoding = "utf-8"
    tool = "pipelineNoisy"
    ...
    token = get_token()
    conditional_info('[INFO] Pipeline tool: {}'.format(tool), args.quiet)
    conditional_info('[INFO] File I/O encoding: {}'.format(encoding), args.quiet)
    conditional_info('[INFO] Output destination: .{}{}'.format(os.sep, output_path), args.quiet)

    start_time = time.time()

    caller = PipelineCaller(tool, text, token, args.processing_type)
